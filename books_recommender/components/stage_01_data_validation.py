...
class DataValidation:
    def __init(self, app_config = Appconfiguration()):
        try:
    ...
    def preprocess_data(self):
        try:
            ...
            books =pd.read_csv(self.data_validation_config.books_csv_file_csv_file,sep=';', error_bad_lines=False, encoding='latin-1')

            logging.info(f" Shape of ratings data file: {ratings.shape}")
            logging.info(f" Shape of books data file: {books.shape}")
            
            # Here image URL columns is important for poster.So, we will keep it
            ...
            # Saving final_rating object for web app
